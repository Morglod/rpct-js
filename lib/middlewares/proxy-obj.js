...
exports.PROXY_OBJ_MARKER = exports.proxyObjMiddleware = void 0;
...
exports.proxyObjMiddleware = () => {
    ...
    const pack = (arg, argI, rid) => {
        if (typeof arg === 'object' && 'PROXY_OBJ_MARKER' in arg && arg['PROXY_OBJ_MARKER'] === exports.PROXY_OBJ_MARKER) {
            if (api.config.debug)
    ...
    const unpack = (arg, argI, rid) => {
        if (arg.type === api_1.ApiProtocolArgTypeFlag.proxy) {
            if (api.config.debug)
